...
__name(isSignalPayload, "isSignalPayload");
function isTelemetrySample(v) {
  return isRec(v) && typeof v.seq === "number" && typeof v.t === "number";
}
__name(isTelemetrySample, "isTelemetrySample");
function isGameMessage(v) {
  if (!isRec(v)) return false;
  switch (v.kind) {
    case "aim-lock":
    ...
    case "profile":
      return typeof v.name === "string" && v.name.length <= 24 && (v.face === void 0 || typeof v.face === "string" && v.face.length <= 4e4 && v.face.startsWith("data:image/"));
    case "swing":
...
__name(isGameMessage, "isGameMessage");
...
__name(isClientMessage, "isClientMessage");

// party/server.ts
var WeeGolfRoom = class extends Server {
  static {
    __name(this, "WeeGolfRoom");
  }
  onConnect(conn, ctx) {
    ...
    const info = { peerId, role, joinedAt: Date.now() };
    conn.setState(info);
  ...
  peers(excludePeerId) {
    return this.connections().map((c) => c.state).filter((s) => s != null && s.peerId !== excludePeerId);
  }
  findHost() {
...
// .wrangler/tmp/bundle-vXfsdd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-vXfsdd/middleware-loader.entry.ts
